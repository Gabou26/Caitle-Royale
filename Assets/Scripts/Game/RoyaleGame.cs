...
public class RoyaleGame : GameManager
{
    ...
    private IEnumerator AjustementCamera()
    {
        ...
        if (targetGroup.IsEmpty)
        {
            ...
            if (target != null && target.gameObject.activeInHierarchy) //Added but not tested 100%
                targetGroup.AddMember(target.GetComponent<MousePoint>().mousePoint, 1f, 0f);
        }
    }

    public bool IsAnyPlayerAlive() { return royaleModeUILife.IsAnyPlayerAlive(this); }
    public int GetPlayerCount() { return playerAlive; }
    public bool IsGamePlaying() { return isPlaying; }
