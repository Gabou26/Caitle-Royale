...
public class GameManager : MonoBehaviour
{
    ...
    {
        /*bool jakIsKindaSleepySinceHeMadeLike3SongsToday = true;
        bool jakShouldGoToSleep = false;
        if (jakIsKindaSleepySinceHeMadeLike3SongsToday)
            jakShouldGoToSleep = true;
        if (jakShouldGoToSleep)
            print(" Jak Go to sleep :) ");*/
    ...
    Score MostKills()
    {
        ...
        foreach (var playerScore in playersScores)
        {
            if (playerScore.Value.killScore > topScore.killScore)
                topScore = playerScore.Value;
            else if (playerScore.Value.killScore == topScore.killScore && playerScore.Value.damageScore > topScore.damageScore)
                topScore = playerScore.Value;
        }
    ...
    private IEnumerator ShowScore()
    {
        ...
        foreach (var playerScore in playersScores)
        {
            resultsPlayerText[i].text = playerScore.Value.name + " : " + playerScore.Value.killScore + " - " +
                                        playerScore.Value.deathScore + " - " + playerScore.Value.suicideScore;
            i++;
        }
